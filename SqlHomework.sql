...
create table address (address_id Int NOT NULL, address VARCHAR(30), address2 VARCHAR(30), district VARCHAR(30),city_id INT);
...
select s.first_name, s.last_name, sum(p.amount) from staff as s join payment as p on s.staff_id=p.staff_id group by s.first_name having p.payment_date between '2005-08-01' and '2005-09-01';
...
select c.first_name, c.last_name, sum(p.amount) from customer as c join payment as p on c.customer_id=p.customer_id group by c.first_name order by c.last_name asc;
